      ...
      <div className="mova-modal-wrap">
        ...
        <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.98 }} className="mova-modal relative z-10 p-5">
          ...
          <div className="mt-4 max-h-72 space-y-2 overflow-y-auto">
            {filtered.map((city) => (
              <button key={city} onClick={() => onSelect(city)} className={`flex w-full items-center justify-between rounded-[0.85rem] px-4 py-3 text-left text-sm font-semibold ${city === value ? 'bg-[#FD7407] text-[#111215]' : 'bg-white/[0.06] text-white/78'}`}>
    ...
    <main className="mova-screen">
      <section className="relative mova-mobile h-screen overflow-hidden pb-24">
        ...
        <header className="pointer-events-auto relative z-20 flex items-center justify-between px-5 pt-7 text-[var(--mova-text)]">
          <button onClick={() => setCityOpen(true)} className="inline-flex h-10 items-center gap-2 rounded-[0.16rem] border border-white/10 bg-[#111215]/82 px-4 text-sm font-semibold text-white shadow-[0_12px_32px_rgba(0,0,0,.24)] backdrop-blur-md">{city}<FiChevronDown className="text-[#F9A809]" /></button>
          <div className="flex gap-2">
            <button onClick={requestLocation} className="grid h-10 w-10 place-items-center rounded-[0.16rem] border border-white/10 bg-white/90 text-lg text-[#111215] shadow-[0_12px_32px_rgba(0,0,0,.22)] backdrop-blur-md"><FiNavigation /></button>
            <button onClick={() => setFiltersOpen(true)} className="grid h-10 w-10 place-items-center rounded-[0.16rem] border border-white/10 bg-[#111215]/88 text-lg text-white/84 shadow-[0_12px_32px_rgba(0,0,0,.22)] backdrop-blur-md"><FiSliders /></button>
          </div>
        </header>
          ...
          <div className="mova-modal-wrap">
            ...
            <motion.div initial={{ opacity: 0, y: 18, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="mova-modal relative z-10 p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {categories.map((chip) => (
                  <button key={chip} onClick={() => setActiveCategory(chip)} className={`rounded-[0.6rem] px-3 py-2 text-xs font-black ${activeCategory === chip ? 'bg-[#0869D0] text-white' : 'bg-white/[0.07] text-white/62'}`}>{chip}</button>
                ))}
              </div>
              <button onClick={() => setFiltersOpen(false)} className="mt-6 h-12 w-full rounded-[0.9rem] bg-[#FD7407] text-sm font-black text-[#111215] hover:bg-[#F9A809]">Aplicar filtros</button>
            </motion.div>
          </div>
        )}
