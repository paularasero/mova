...
const accentIcon = L.divIcon({
  className: '',
  html: '<div style="width:34px;height:34px;border-radius:999px;background:var(--mova-accent);box-shadow:0 0 0 8px rgba(123,97,255,.16),0 12px 30px rgba(0,0,0,.18);display:grid;place-items:center;color:white;font-weight:900;border:2px solid rgba(255,255,255,.9)">•</div>',
  iconSize: [34, 34],
  iconAnchor: [17, 17],
  popupAnchor: [0, -18],
});
  ...
  return (
    <div className="w-56 overflow-hidden rounded-2xl bg-[var(--mova-surface)] text-[var(--mova-text)]">
      ...
      <div className="p-3">
        ...
        <p className="mt-2 text-xs font-semibold text-[var(--mova-accent)]">{experience.category}</p>
        <div className="mt-3 flex gap-2">
          <Link to={`/plan/${experience.id}`} className="flex-1 rounded-full bg-[var(--mova-accent)] px-3 py-2 text-center text-xs font-bold text-white">Ver plan</Link>
          <button onClick={() => onSave(experience.id)} className="grid h-8 w-8 place-items-center rounded-full bg-black/10 text-[var(--mova-accent)]"><FiBookmark /></button>
        </div>
      </div>
    </div>
  );
      ...
      <section className="relative mova-mobile h-screen overflow-hidden pb-24">
        <MapContainer center={center} zoom={13} scrollWheelZoom className="absolute inset-0 z-0 h-full w-full">
          ...
          {filtered.map((experience, index) => (
            <Marker key={experience.id} position={getCoords(experience, index)} icon={accentIcon} eventHandlers={{ click: () => setSelected(experience) }}>
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 h-44 bg-gradient-to-b from-white/86 to-transparent dark:from-black/54" />
        <header className="pointer-events-auto relative z-20 flex items-center justify-between px-5 pt-7 text-[var(--mova-text)]">
          <Link to="/home" className="grid h-11 w-11 place-items-center rounded-full bg-[var(--mova-surface)]/88 text-xl shadow-[0_10px_25px_rgba(17,17,17,0.08)] backdrop-blur-xl"><FiArrowLeft /></Link>
          <button className="inline-flex h-11 items-center gap-2 rounded-full bg-[var(--mova-surface)]/90 px-5 text-sm font-bold shadow-[0_10px_25px_rgba(17,17,17,0.08)] backdrop-blur-xl">{city}<FiChevronDown className="text-[var(--mova-accent)]" /></button>
          <div className="flex gap-2">
            <button className="grid h-11 w-11 place-items-center rounded-full bg-[var(--mova-surface)]/90 text-xl shadow-[0_10px_25px_rgba(17,17,17,0.08)] backdrop-blur-xl"><FiNavigation /></button>
            <button className="grid h-11 w-11 place-items-center rounded-full bg-[var(--mova-surface)]/90 text-xl shadow-[0_10px_25px_rgba(17,17,17,0.08)] backdrop-blur-xl"><FiSliders /></button>
          </div>
        </header>

        <div className="pointer-events-auto relative z-20 mx-5 mt-4 flex h-12 items-center gap-3 rounded-full bg-[var(--mova-surface)]/92 px-4 text-sm text-[var(--mova-text)] shadow-[0_10px_25px_rgba(17,17,17,0.08)] backdrop-blur-xl">
          <FiSearch className="text-[var(--mova-accent)]" />
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar en el mapa" className="w-full bg-transparent outline-none placeholder:text-[var(--mova-muted)]" />
        </div>
        <div className="pointer-events-auto relative z-20 mx-5 mt-3 flex gap-2 overflow-x-auto">
          {['Todos', 'Night', 'Food', 'Chill', 'Outdoor', 'Música'].map((chip) => (
            <button key={chip} onClick={() => setActiveCategory(chip)} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold backdrop-blur-xl ${activeCategory === chip ? 'bg-[var(--mova-accent)] text-white' : 'bg-[var(--mova-surface)]/90 text-[var(--mova-muted)]'}`}>{chip}</button>
          ))}
        </div>
            ...
            <div className="flex gap-3">
              <Link to={`/plan/${selected.id}`} className="flex min-w-0 flex-1 gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3 flex gap-2">
                    <button className="rounded-full bg-[var(--mova-card)] px-3 py-1.5 text-xs font-bold">Más tarde</button>
                    <button className="rounded-full bg-[var(--mova-accent)] px-3 py-1.5 text-xs font-bold text-white">Me sumo</button>
                  </div>
                </div>
              </Link>
              <button onClick={() => save(selected.id)} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[var(--mova-accent-soft)] text-[var(--mova-accent)]"><FiHeart /></button>
            </div>
            {message && <p className="mt-3 text-xs font-semibold text-[var(--mova-accent)]">{message}</p>}
          </motion.div>
        )}

        <section className="absolute bottom-24 left-0 right-0 z-20 px-5">
          <h3 className="mb-3 text-sm font-semibold text-[var(--mova-text)] drop-shadow">Planes cerca de ti</h3>
          <div className="flex gap-3 overflow-x-auto pb-1">
            {filtered.slice(0, 8).map((item) => (
              <button key={item.id} onClick={() => setSelected(item)} className={`photo-card relative h-28 w-44 shrink-0 overflow-hidden rounded-2xl border text-left ${selected?.id === item.id ? 'border-[var(--mova-accent)]' : 'border-white/10'}`}>
        ...
        <div className="absolute left-5 top-40 z-20 rounded-full bg-[var(--mova-surface)]/90 px-3 py-2 text-xs text-[var(--mova-muted)] shadow-[0_10px_25px_rgba(17,17,17,0.08)] backdrop-blur-xl">
          <FiMapPin className="mr-1 inline text-[var(--mova-accent)]" /> {filtered.length} planes
        </div>
      </section>
    </main>
  );
