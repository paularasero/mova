    ...
    <main className="mova-screen">
      <section className="mova-mobile px-5 pb-32 pt-7">
        ...
        <section className="mt-8 text-center">
          <div className="mova-organic mx-auto grid h-32 w-32 place-items-center overflow-hidden bg-[#FF74C8] p-1 shadow-[0_12px_0_rgba(125,255,114,.2)] ring-2 ring-[#FFD84D] ring-offset-4 ring-offset-[var(--mova-bg)]">
          ...
          <div className="mova-sticker mx-auto mt-3 inline-flex -rotate-1 items-center gap-2 rounded-full bg-[#FFD84D] px-4 py-2 text-xs font-black">
            <FiStar /> Urban Explorer · Nivel {Math.max(1, Math.floor(points / 500) + 1)}
          </div>
          <button className="mt-4 rounded-[1rem] border border-white/12 bg-white/10 px-5 py-2 text-sm font-black">Editar perfil</button>
        </section>
        ...
        <section className="mt-6">
          ...
          <div className="flex flex-wrap gap-2">
            {vibes.map((vibe, index) => <span key={vibe} className={`rounded-[0.95rem] px-4 py-2 text-xs font-black text-[#0B0B0F] ${index % 3 === 0 ? 'bg-[#7DFF72]' : index % 3 === 1 ? 'bg-[#67C8FF]' : 'bg-[#FF74C8]'}`}>{vibe}</span>)}
          </div>
        </section>
        ...
        <div className="mt-6 flex gap-2">
          {tabs.map((item) => (
            <button key={item} onClick={() => setTab(item)} className={`rounded-[1rem] px-4 py-2 text-xs font-black ${tab === item ? 'bg-[#7DFF72] text-[#0B0B0F]' : 'bg-white/[0.07] text-[var(--mova-muted)]'}`}>{item}</button>
          ))}
        </div>
