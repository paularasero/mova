...
import { FiBookmark, FiEdit3, FiGrid, FiSettings, FiStar, FiUsers } from 'react-icons/fi';
...
function GraphicAvatar({ user }) {
  ...
  return (
    <div className="relative mx-auto h-32 w-32 overflow-hidden rounded-[0.35rem] border border-[#F2EDEA]/14 bg-[#111215] shadow-[0_18px_52px_rgba(0,0,0,.34)]">
      <div className="absolute -left-8 -right-8 -top-10 h-28 rounded-full bg-[linear-gradient(180deg,#FD7407,#F9A809)] opacity-88" />
      <div className="absolute -bottom-12 -left-8 h-28 w-28 rounded-t-full bg-[#04533E]/82" />
      <div className="absolute -bottom-10 -right-8 h-32 w-32 rounded-t-full bg-[#0869D0]/58" />
      <div className="mova-print-texture absolute inset-0 opacity-55" />
      {customPhoto ? (
...
function PosterHighlight({ label, value, color, index }) {
  return (
    <motion.article whileTap={{ scale: 0.98 }} className="relative h-28 min-w-32 overflow-hidden rounded-[0.35rem] border border-white/10 bg-[#111215] p-3">
      <div className="absolute -right-8 -top-8 h-20 w-20 rounded-full opacity-28" style={{ backgroundColor: color }} />
      <div className="absolute -bottom-8 left-2 h-24 w-24 rounded-t-full opacity-20" style={{ backgroundColor: palette[(index + 2) % palette.length] }} />
      <div className="mova-print-texture absolute inset-0 opacity-28" />
  ...
  return (
    <main className="mova-screen">
      <section className="mova-mobile relative overflow-hidden px-5 pb-32 pt-7">
        <div className="pointer-events-none absolute -right-24 top-28 h-64 w-64 rounded-full bg-[#FD7407]/7 blur-2xl" />
        <div className="pointer-events-none absolute -left-20 top-80 h-56 w-56 rounded-t-full bg-[#0869D0]/6 blur-xl" />
        ...
        <motion.section initial={{ opacity: 0, y: 18, filter: 'blur(8px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} className="relative z-10 mt-7 text-center">
          ...
          <div className="mt-4 inline-flex items-center gap-2 rounded-[0.16rem] border border-white/12 bg-[#F9A809] px-3 py-2 text-xs font-black uppercase tracking-[0.08em] text-[#111215]">
            <FiStar /> Urban Explorer · Nivel {Math.max(1, Math.floor(points / 500) + 1)}
          </div>
          <button className="ml-2 inline-flex items-center gap-2 rounded-[0.16rem] border border-white/12 bg-white/[0.07] px-3 py-2 text-xs font-black text-white/72">
            <FiEdit3 /> Editar
          </button>
        </motion.section>
        ...
        <section className="relative z-10 mt-7">
          ...
          <div className="flex flex-wrap gap-2">
            {vibes.map((vibe, index) => (
              <span key={vibe} className="rounded-[0.16rem] border px-3 py-2 text-xs font-black uppercase tracking-[0.06em] text-white/74" style={{ borderColor: `${palette[index % palette.length]}88` }}>
                {vibe}
              </span>
            ))}
          </div>
