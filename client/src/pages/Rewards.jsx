    ...
    <main className="mova-screen">
      <section className="mova-mobile px-5 pb-28 pt-8">
        <header className="flex items-center justify-between">
          ...
          <span className="grid h-11 w-11 place-items-center rounded-full bg-white/[0.08] text-[var(--mova-accent)]"><FiStar /></span>
        </header>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-7 rounded-[1.7rem] border border-white/8 bg-white/[0.06] p-5">
          ...
          <div className="mt-5 rounded-2xl border border-white/8 bg-black/25 p-4">
            ...
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10">
              <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} className="h-full rounded-full bg-[var(--mova-accent)]" />
            </div>
        ...
        <div className="mt-3 grid grid-cols-3 gap-3">
          {achievements.map((item) => (
            <div key={item.label} className="rounded-2xl bg-white/[0.055] p-3 text-center">
              <span className="mx-auto grid h-10 w-10 place-items-center rounded-full bg-[var(--mova-accent-soft)] text-[var(--mova-accent)]"><item.icon /></span>
        ...
        <button className="mt-3 flex w-full items-center gap-3 rounded-[1.4rem] border border-white/8 bg-white/[0.055] p-4 text-left">
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[var(--mova-accent-soft)] text-[var(--mova-accent)]"><FiCoffee /></span>
          <span className="flex-1">
            <span className="block text-sm font-semibold">10% OFF en cafés</span>
            <span className="text-xs text-white/42">2.000 pts</span>
          </span>
          <span className="rounded-full bg-[var(--mova-accent)] px-3 py-2 text-xs font-bold text-white">Canjear</span>
        </button>
      </section>
    </main>
  );
