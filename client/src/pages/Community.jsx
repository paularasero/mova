...
const tabs = ['Actividad', 'Personas', 'Mensajes'];
const avatars = [
    ...
    <main className="mova-screen">
      <section className="mova-mobile px-5 pb-32 pt-8">
        ...
        <div className="mt-6 flex gap-2 overflow-x-auto">
          {tabs.map((tab) => <button key={tab} onClick={() => setActive(tab)} className={`rounded-full px-4 py-2 text-sm font-semibold ${active === tab ? 'bg-[#C8FF3D] text-black' : 'mova-card text-[var(--mova-muted)]'}`}>{tab}</button>)}
        </div>
        ...
        {active === 'Personas' && (
          <section className="mt-6">
            <label className="flex h-14 items-center gap-3 rounded-full border border-[var(--mova-border)] bg-[var(--mova-card)] px-4">
              <FiSearch className="text-[#8ab500]" />
            ...
            <div className="mt-5 space-y-3">
              {visibleUsers.map((user) => {
                ...
                return (
                  <article key={user.id} className="mova-card flex items-center gap-3 rounded-[1.5rem] p-4">
                    <img src={user.avatar} alt="" className="h-14 w-14 rounded-full bg-[#C8FF3D]/14 object-cover" />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold">{user.name}</p>
                      <p className="truncate text-xs text-[var(--mova-muted)]">@{user.username} · {user.city}</p>
                    </div>
                    <button onClick={() => toggleFollow(user.id)} className={`rounded-full px-4 py-2 text-xs font-bold ${isFollowing ? 'bg-white/[0.08] text-[var(--mova-text)]' : 'bg-[#C8FF3D] text-black'}`}>
                      {isFollowing ? 'Siguiendo' : 'Seguir'}
                    </button>
                  </article>
                );
              })}
              {!visibleUsers.length && <p className="rounded-2xl bg-white/[0.06] px-4 py-4 text-sm text-[var(--mova-muted)]">No encontramos personas con esa búsqueda.</p>}
            </div>
          </section>
        )}
          ...
          <section className="mt-6">
            <div className="flex gap-3 overflow-x-auto pb-2">
              {conversations.map((conversation) => (
                <button key={conversation.name} onClick={() => setChat(conversation)} className={`min-w-36 rounded-[1.35rem] border p-3 text-left ${chat.name === conversation.name ? 'border-[#C8FF3D] bg-[#C8FF3D]/12' : 'border-[var(--mova-border)] bg-[var(--mova-card)]'}`}>
            ...
            <div className="mt-4 rounded-[1.7rem] border border-[var(--mova-border)] bg-[var(--mova-card)] p-4">
              ...
              <div className="mt-4 space-y-2">
                {messages.map((message, index) => (
                  <p key={`${message.text}-${index}`} className={`max-w-[82%] rounded-2xl px-4 py-2 text-sm ${message.from === 'Yo' ? 'ml-auto bg-[#C8FF3D] text-black' : 'bg-white/[0.08]'}`}>{message.text}</p>
                ))}
              </div>
              <div className="mt-4 flex gap-2">
                ...
                <button onClick={sendMessage} className="grid h-11 w-11 place-items-center rounded-full bg-[#C8FF3D] text-black"><FiSend /></button>
              </div>
            </div>
          </section>
        )}
