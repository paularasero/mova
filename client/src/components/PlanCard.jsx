...
export default function PlanCard({ plan, compact = false }) {
  ...
  return (
    <motion.article whileTap={{ scale: 0.98 }}>
      <Link to={`/plan/${plan.id}`} className={`photo-card group relative block overflow-hidden rounded-[0.45rem] border border-white/12 bg-white/[0.06] shadow-[0_10px_0_rgba(253, 116, 7,.10),0_18px_50px_rgba(0,0,0,0.34)] ${compact ? 'h-44' : 'h-60'}`}>
        ...
        <span className="mova-sticker absolute right-3 top-3 rounded-[0.16rem] bg-[#04533E] px-2.5 py-1 text-[10px] font-black">{category}</span>
        <div className="absolute inset-x-0 bottom-0 p-4">
          ...
          <div className="mt-3 flex items-center justify-between text-xs font-semibold text-white/76">
            <span className="rounded-[0.16rem] bg-[#FD7407] px-3 py-1 text-[#111215]">{category}</span>
